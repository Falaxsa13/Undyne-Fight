...
void drawFullScreenImageDMA(const u16 *image)
{
  ...
  DMA[3].cnt = imageSize | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
}
...
void drawCenterString(int row, int col, int width, char *str, u16 color)
{
  int len = strlen(str) * 6;

  int textCol = col + (width - len) / 2;

  drawString(row, textCol, str, color);
}